...
while 1:
    ...
    while game:
        ...
        while cube and timer:
            ...
            cv2.putText(img, textu, (posx+(wo//2)-20, posy+(ho//2)+7), font, 1, (0, 0, 0), 2, cv2.LINE_AA)
            ...
            cv2.putText(vertical_img, 'Lives: ', (400, 30), font, 1, (255, 255, 255), 2, cv2.LINE_AA)
        ...
        if vides <= 0:                                                                      #when we run out of lives the game pops the final audio and image, and the prog
            ...
            if count < 100:
                ...
                pygame.mixer.music.load("cha.wav")
                pygame.mixer.music.play(0)
                victori = cv2.imread('+100.jpg', 3)
                cv2.rectangle(victori, (110, 630), (1000, 770), (0, 0, 0), cv2.FILLED)
                cv2.putText(victori, 'WINNER, GOT: ' + str(count) + ' POINTS!', (130, 700), font, 2, (255, 255, 255), 4,cv2.LINE_AA)
                cv2.imshow('img', victori)
                k = cv2.waitKey(30) & 0xff
                time.sleep(19)
    k = cv2.waitKey(30) & 0xff
